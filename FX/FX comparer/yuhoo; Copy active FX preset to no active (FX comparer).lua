-- @noindex
-- @description Copy active FX preset to no active (FX comparer)
-- @author yuhoo
-- @version 1.0.0

local trackidx, itemidx, takeidx, fx, guid, fx_chunk, ret, name, fx_name
...
if not ret then
  ...
  ret, fx_chunk = reaper.GetProjExtState(proj, observer_settings.ext_name_string, observer_settings.key)
  trackidx, itemidx, takeidx, fx = fx_chunk:match('(%S+)%s-(%S+)%s-(%S+)%s-(%S+)')
...
if itemidx == -1 then
  if reaper.TrackFX_GetOpen(track, fx) or settings.ignore_visibility then
    ...
    reaper.Undo_EndBlock2(proj, "FX comparer: Copy preset " .. name .. ": " .. fx_name, -1)
  end
else
  ...
  if reaper.TakeFX_GetOpen(take, fx) or settings.ignore_visibility then
    ...
    fx_name = yh.GetOnlyFXName(fx_name)
    reaper.Undo_EndBlock2(proj, "FX comparer: Copy preset: Item on track " .. name .. ": " .. fx_name, -1)
  end
end
